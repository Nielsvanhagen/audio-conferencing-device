...
String device_name = "Audio Device 2";
  ...
  if (WiFi.status() == WL_CONNECTED)
     {
          ...
          tft.setTextColor(TFT_WHITE);  tft.setTextSize(1); tft.setTextFont(2);
          tft.print("IP addres : ");
...
void wifi_setup_layout()
{
  ...
  tft.println("- Starting on name: Audio Conferencing Device 2");
  //creating auto AP for connection
  if(!wifiManager.autoConnect("Audio Conferencing Device 2")){ 
...
void open_call()
{
  ...
  tft.setTextSize(1);
  tft.setTextDatum(MC_DATUM);
  tft.drawString("DETAILS", MUTEAMPBUTTON_X + (MUTEAMPBUTTON_W / 2) + 1, MUTEAMPBUTTON_Y + (MUTEAMPBUTTON_H / 2));
  tft.drawRect(FRAME_X5, FRAME_Y5, FRAME_W5, FRAME_H5, TFT_BLACK);
  
}

//listener layout function
...
//calibrate function if display is not calibrated
